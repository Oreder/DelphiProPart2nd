...
var
  ...
  MX,MY: Integer;
  Car2Color: TColor;
  TurnOver, EnableMouse, Collision: Boolean;
...
begin
  ...
  if I mod 6 = 0 then
    I := 0;
  J := J + 1;
  if (J mod 100 = 0) or (Car2Color = (RGB(0,0,0))) then
  begin
    J := 0;
  ...
  Distance.X := Abs(Car1.X - Car2.X);
    ...
    begin
      Collision := True;
      ShowMessage('Ты только что машину.');
      EnableMouse := false;
      Collision := False;
    end;
    ...
    begin
      Car1.X := MX;
      Car1.Y := MY+Round(Pi*Sqrt(2*K+1)/4*Sin(I))-8*K;;
    end;
...
//Image1.Picture := nil;
  ...
  begin
    ...
    Pen.Color := clBlack;
    Brush.Color := clGray;
    Rectangle(4*PartX, 0, 16*PartX, Height);
    Brush.Color := clgreen;
    Rectangle(0,0,4*PartX,Height);
    Rectangle(16*PartX,0,Width,Height);
    {FloodFill(2*PartX, 1, clwhite, fssurface);
    FloodFill(18*PartX, 1, clwhite, fssurface);
    FloodFill(2*PartX, 160, clgreen, fssurface);
    FloodFill(18*PartX, 160, clgreen, fssurface);}
    brush.Color := clgray;
...
procedure TForm1.Compute(Sender: TObject);
begin
